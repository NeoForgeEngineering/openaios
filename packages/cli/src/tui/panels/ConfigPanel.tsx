...
export function ConfigPanel({
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch(
        `${baseUrl}/api/config/agents/${encodeURIComponent(editData.name)}`,
        {
          method: 'PATCH',
          ...
          body: JSON.stringify({
            ...
            permissions: {
              allow: editData.permissions.allow,
              deny: editData.permissions.deny,
            },
          }),
        },
      )
      if (res.status === 204) {
        setAgents((prev) =>
          prev.map((a) => (a.name === editData.name ? editData : a)),
        )
        setToast('✓ Saved')
  ...
  useInput((input, key) => {
    ...
    if (focusField === 'skills') {
      ...
      if (key.downArrow)
        setFocusSkillIdx((i) => Math.min(skills.length - 1, i + 1))
      if (input === ' ') {
    ...
    if (focusField === 'browser' && input === ' ' && editData) {
      setEditData({
        ...
        capabilities: {
          ...
          browser: !editData.capabilities.browser,
        },
      })
    }
  })
        ...
        <Box flexDirection="column" gap={1}>
          ...
          <Box flexDirection="column">
            ...
            <Text color="gray" dimColor>
              {editData.persona.slice(0, 60)}
              {editData.persona.length > 60 ? '…' : ''}
            </Text>
          </Box>

          <Box flexDirection="column">
            <Text
              bold={focusField === 'skills'}
              color={focusField === 'skills' ? 'white' : 'gray'}
            >
          ...
          <Box flexDirection="column">
            <Text
              bold={focusField === 'browser'}
              color={focusField === 'browser' ? 'white' : 'gray'}
            >
              Capabilities:{' '}
              {focusField === 'browser' ? '(Space=toggle Tab=next)' : ''}
            </Text>
            <Text color="gray">
              {' '}
              [{editData.capabilities.browser ? 'x' : ' '}] Browser
            </Text>
          </Box>

          <Box flexDirection="column">
            <Text
              bold={focusField === 'allow'}
              color={focusField === 'allow' ? 'white' : 'gray'}
            >
              Permissions allow:
            </Text>
            <Text color="gray">
              {' '}
              {editData.permissions.allow.join(', ') || '(none)'}
            </Text>
          </Box>

          <Box flexDirection="column">
            <Text
              bold={focusField === 'deny'}
              color={focusField === 'deny' ? 'white' : 'gray'}
            >
              Permissions deny:
            </Text>
            <Text color="gray">
              {' '}
              {editData.permissions.deny.join(', ') || '(none)'}
            </Text>
          </Box>
          ...
          <Box gap={2}>
            ...
            {toast && (
              <Text color={toast.startsWith('✓') ? 'green' : 'red'}>
                {toast}
              </Text>
            )}
          </Box>

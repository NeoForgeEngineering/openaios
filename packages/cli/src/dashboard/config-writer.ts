...
export function patchAgentInConfig(configPath: string, agentName: string, patch: AgentPatch): void {
  const raw = readFileSync(configPath, 'utf-8')
  const doc = parseDocument(raw)
  const agents = doc.get('agents')
  if (!isSeq(agents)) throw new Error('agents must be a sequence')

  const idx = agents.items.findIndex((item) => isMap(item) && item.get('name') === agentName)
  if (idx === -1) throw new Error(`Agent "${agentName}" not found in config`)

  if (patch.persona !== undefined) doc.setIn(['agents', idx, 'persona'], patch.persona)
  if (patch.skills !== undefined) doc.setIn(['agents', idx, 'skills'], patch.skills)
